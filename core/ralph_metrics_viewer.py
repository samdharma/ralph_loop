...
def render_html(stats: Dict[str, Any]) -> str:
    ...
    tier_rows = "\n".join(
        ...
        f"<td>{stats['tier_stats'][tier]['avg_min'] * stats['tier_stats'][tier]['count']:.1f}</td></tr>"
        for tier in sorted(stats["tier_stats"])
    )
    type_rows = "\n".join(
...
<p>Pass rate: <span class="metric">{stats['passes']} passed</span> / <span class="metric">{stats['fails']} failed</span> ({stats['pass_rate']:.1f}% / {stats['fail_rate']:.1f}%)</p>
